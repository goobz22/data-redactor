...
declare class BaseScenario implements Scenario {
    ...
    strategy: RedactionStrategy;
    enabled: boolean;
    constructor(name: string, pattern: RegExp, captureGroup?: number, strategy?: RedactionStrategy, enabled?: boolean);
    findAll(text: string): Match[];
    setStrategy(strategy: RedactionStrategy): void;
    setEnabled(enabled: boolean): void;
}
...
interface PatternSegment {
    regex: string;
    description: string;
    type: 'digit' | 'lower' | 'upper' | 'hex' | 'whitespace' | 'literal' | 'uuid' | 'ipv4' | 'mac' | 'unknown';
    isVariable: boolean;
    minLength: number;
    maxLength: number;
 ...
 */
declare function mergeAdjacentPatterns(segments: PatternSegment[]): PatternSegment[];
 ...
 */
declare function addWordBoundaries(regex: string, addBoundaries?: boolean): string;
/**
 * Validate that a regex compiles and matches the original sample
 */
declare function validateRegex(regex: string, sample: string): {
    valid: boolean;
    error?: string;
    matches: boolean;
};
 ...
 */
declare function generateFromSample(sample: string, options?: GenerateOptions): GeneratedPattern;
/**
 ...
 */
declare function testPattern(regex: string, samples: string[]): Array<{
    sample: string;
    matches: boolean;
    matchedText?: string;
}>;
